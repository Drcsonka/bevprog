...
main (int argc, char *argv[])
{
    ...
    */
    ...
    std::string s = argv[1];
        ...
        if (s == "-bk")
        {
            kiFile << "A fába bekerült 0-k száma : " <<binFa.getNulla() << std::endl;
            kiFile << "A fába bekerült 1-esek száma : " <<binFa.getEgy() << std::endl;
        }
        else if( s == "-pr")
        {
            kiFile <<"Vizsgált 1-esek száma : " << binFa.getOsszEgy() << std::endl ;
            kiFile <<"Vizsgált 0-k száma : " << binFa.getOsszNulla() << std::endl ;
        }
        else if( s == "-s")
        {
        kiFile <<"A vizsgált egyek "<< stat(binFa.getEgy(),binFa.getOsszEgy()) << "%-a került be a fába"<< std::endl;
        kiFile <<"A vizsgált nullák "<< stat(binFa.getNulla(),binFa.getOsszNulla()) << "%-a került be a fába"<< std::endl;
    ...
    //kiFile << "nullak = "<<binFa.getNulla()<<std::endl;
    //kiFile << "egyek = "<<binFa.getEgy()<<std::endl;

    kiFile.close ();
    beFile.close ();
    //std::cout<<binFa.getEgy()<<" "<<binFa.getNulla();
    return 0;
}